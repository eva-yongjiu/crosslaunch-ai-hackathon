...
import { appPath, createProject, deleteProject, getRuntimeStatus, listProjects, loadWorkspace, runWorkflow, saveWorkspace, uploadAsset } from "../lib/api-client";
...
export function ExperienceStudio() {
  ...
  const execute = async (action: "analyze" | "confirm_truth" | "generate" | "translate" | "scan" | "apply_fixes" | "optimize_finding" | "optimize_all" | "regenerate_asset", next?: Space, assetId?: string, findingId?: string, generateAll = false) => {
    if (!workspace) return;
    setBusy(true); setActiveAction(action); setError(""); setMessage("");
    try {
      const result = await runWorkflow(workspace.project.id, action, workspace, action === "generate" && !generateAll ? channel : undefined, assetId, findingId);
      setSavedWorkspace(result.workspace);
  ...
  return <main className="experience-shell real-app">
    <header className="experience-header">
      ...
      <nav aria-label="主导航">
        {(["projects", "truth", "create", "compliance"] as Space[]).map((item, index) => <button key={item} className={space === item ? "active" : ""} disabled={(!workspace && item !== "projects") || (item === "compliance" && !hasGeneratedContent(workspace))} onClick={() => setSpace(item)}><i>{index + 1}</i>{["项目", "商品事实", "内容创作", "合规导出"][index]}</button>)}
      </nav>
    ...
    {space === "projects" && <ProjectsHome projects={projects} busy={busy} onOpen={openProject} onDelete={removeProject} onCreated={(next) => { setSavedWorkspace(next); setProjects((items) => [next.project, ...items]); setChannel(next.project.channels[0]); setSpace("truth"); }} onError={setError} />}
    ...
    {workspace && space === "create" && <CreationStudio workspace={workspace} setWorkspace={setWorkspace} channel={channel} setChannel={setChannel} view={view} setView={setView} busy={busy} aiReady={aiReady} hasUnsavedChanges={hasUnsavedChanges} outputLanguage={workspace.outputLanguage ?? "bilingual"} focusTarget={focusTarget} onLanguageChange={changeOutputLanguage} onTranslate={() => execute("translate")} onGenerate={() => execute("generate")} onGenerateAll={() => execute("generate", undefined, undefined, undefined, true)} onRegenerateAsset={(assetId) => execute("regenerate_asset", undefined, assetId)} onReplaceAsset={replaceAsset} onSave={() => save("编辑渠道内容")} onCompliance={() => setSpace("compliance")} />}
...
function ProjectsHome({ projects, busy, onOpen, onDelete, onCreated, onError }: { projects: LaunchProject[]; busy: boolean; onOpen: (id: string) => void; onDelete: (id: string) => void; onCreated: (workspace: ProjectWorkspace) => void; onError: (value: string) => void }) {
  ...
  return <section className="home-space real-home">
    ...
    <div className="recent-block"><div className="section-title"><div><span>我的项目</span><h2>已保存项目</h2></div><b>{projects.length} 个</b></div>
      {projects.length ? <div className="project-row real-projects">{projects.map((project) => <article className="project-tile current" key={project.id}><button className="project-open" onClick={() => onOpen(project.id)} disabled={busy}><div className="tile-visual neutral">{project.productName.slice(0, 1).toUpperCase()}</div><div><span className={`project-state ${project.status === "completed" ? "done" : "review"}`}>{statusName(project.status)}</span><h3>{project.name}</h3><p>{project.channels.map((item) => channelNames[item]).join(" · ")}</p><small>更新于 {new Date(project.updatedAt).toLocaleString("zh-CN")}</small></div><b>打开 →</b></button><button type="button" className="project-delete" onClick={() => onDelete(project.id)} disabled={busy}>删除项目</button></article>)}</div> : <div className="empty-panel"><b>还没有项目</b><p>上方创建的将是空白、可持久化的真实项目。</p></div>}
    </div>
  </section>;
}
...
function TruthWorkbench({ workspace, setWorkspace, busy, aiReady, analyzing, hasUnsavedChanges, outputLanguage, onLanguageChange, onTranslate, onUpload, onAnalyze, onConfirm, onSave }: { workspace: ProjectWorkspace; setWorkspace: (value: ProjectWorkspace) => void; busy: boolean; aiReady: boolean; analyzing: boolean; hasUnsavedChanges: boolean; outputLanguage: OutputLanguage; onLanguageChange: (value: OutputLanguage) => void; onTranslate: () => void; onUpload: (file: File) => void; onAnalyze: () => void; onConfirm: () => void; onSave: () => void }) {
  ...
  return <section className="focused-space"><SpaceIntro number="01" label="商品信息" title="先确认真实商品信息，再让 AI 创作。" text="你可以上传原图让 AI 识别，也可以手动录入。只有你确认过的信息才会用于生成。" />
      <div className="workbench-grid"><label className="product-stage"><input type="file" accept="image/*" disabled={busy} onChange={(event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (file) onUpload(file); event.target.value = ""; }} />{workspace.truth.sourceAsset ? <ResilientImage src={appPath(workspace.truth.sourceAsset.url)} alt="商品原图" /> : <div className="upload-placeholder"><b>上传真实商品原图</b><span>JPG / PNG / WEBP，最大 8MB</span></div>}<span className="stage-chip">原始商品素材</span><button type="button">{workspace.truth.sourceAsset ? "更换图片" : "选择图片"}</button></label>
      <section className="truth-editor real-editor"><header><div><span>商品事实档案</span><input className="title-input" value={workspace.truth.productName} onChange={(event) => updateTruth({ productName: event.target.value })} /></div><div className="truth-header-tools"><label>内容语言<select aria-label="AI输出语言" value={outputLanguage} onChange={(event) => onLanguageChange(event.target.value as OutputLanguage)}><option value="bilingual">中英对照</option><option value="zh-CN">中文优先</option><option value="en-US">English first</option></select></label><strong>{verified}<small>已确认事实</small></strong></div></header>
...
function CreationStudio({ workspace, setWorkspace, channel, setChannel, view, setView, busy, aiReady, hasUnsavedChanges, outputLanguage, focusTarget, onLanguageChange, onTranslate, onGenerate, onGenerateAll, onRegenerateAsset, onReplaceAsset, onSave, onCompliance }: { workspace: ProjectWorkspace; setWorkspace: (value: ProjectWorkspace) => void; channel: Channel; setChannel: (value: Channel) => void; view: CreateView; setView: (value: CreateView) => void; busy: boolean; aiReady: boolean; hasUnsavedChanges: boolean; outputLanguage: OutputLanguage; focusTarget: string | null; onLanguageChange: (value: OutputLanguage) => void; onTranslate: () => void; onGenerate: () => void; onGenerateAll: () => void; onRegenerateAsset: (assetId: string) => void; onReplaceAsset: (assetId: string, file: File) => void; onSave: () => void; onCompliance: () => void }) {
  ...
  return <section className="canvas-space"><div className="canvas-top"><SpaceIntro number="02" label="内容制作" title="每个平台单独生成、单独修改。" text="这里显示当前项目已经生成的真实内容；没有生成的内容会明确提示。" compact /><div className="channel-switch">{workspace.project.channels.map((item) => <button key={item} className={channel === item ? "active" : ""} onClick={() => setChannel(item)}>{channelNames[item]}</button>)}</div></div>
    <div className="canvas-toolbar"><div>{(["assets", "listing", "page"] as CreateView[]).map((item, index) => <button key={item} className={view === item ? "active" : ""} onClick={() => setView(item)}>{["商品图片", "Listing", "详情页"][index]}</button>)}</div><label className="language-control">输出语言<select aria-label="AI输出语言" value={outputLanguage} onChange={(event) => onLanguageChange(event.target.value as OutputLanguage)}><option value="bilingual">中英对照</option><option value="zh-CN">中文优先</option><option value="en-US">English first</option></select></label><span>{workspace.truth.confirmedAt ? "商品信息已确认" : "商品信息未确认"}</span><button className="secondary-action" disabled={busy || !aiReady || !translationNeeded(workspace)} onClick={onTranslate}>{busy ? "翻译中…" : "补齐中文译稿"}</button><button className="regenerate" disabled={busy || !aiReady || !workspace.truth.confirmedAt} onClick={onGenerate}>{aiReady ? (busy ? "生成中…" : `生成 ${channelNames[channel]}`) : "配置 AI 后可生成"}</button><button className="main-action generate-all" disabled={busy || !aiReady || !workspace.truth.confirmedAt} onClick={onGenerateAll}>{aiReady ? (busy ? "正在生成全部渠道…" : "同时生成三个渠道") : "配置 AI 后可生成"}</button></div>
     <div className="creative-layout"><section className="result-canvas">{view === "assets" && <AssetGallery workspace={workspace} channel={channel} busy={busy} aiReady={aiReady} focusTarget={focusTarget} onRegenerate={onRegenerateAsset} onReplace={onReplaceAsset} />}{view === "listing" && <ListingEditor listing={listing} outputLanguage={outputLanguage} setListing={setListing} />}{view === "page" && <PagePreview workspace={workspace} channel={channel} />}</section>
       <aside className="result-rail"><span className="rail-kicker">当前渠道</span><h3>{channelNames[channel]}</h3><div className="result-score"><strong>{listing.score || "—"}</strong><span>{listing.score ? "内容评分" : "尚未生成"}</span></div><div className="rail-stat"><span>已关联商品信息</span><b>{listing.claims.filter((item) => !item.needsEvidence).length}/{listing.claims.length}</b></div><div className="rail-stat"><span>文字风险</span><b>{workspace.findings.filter((finding) => finding.status === "open" && finding.location?.kind === "listing" && finding.location.channel === channel).length || "无"}</b></div><div className="rail-stat"><span>可用图片</span><b>{usableAssetCount}/{assetKinds.length}</b></div><button className="quiet-action" onClick={onSave} disabled={busy || !hasUnsavedChanges}>{hasUnsavedChanges ? "保存修改" : "已保存"}</button><button className="main-action" disabled={!hasGeneratedContent(workspace)} onClick={onCompliance}>进入合规检查 →</button></aside></div>
  </section>;
}
...
function AssetGallery({ workspace, channel, busy, aiReady, focusTarget, onRegenerate, onReplace }: { workspace: ProjectWorkspace; channel: Channel; busy: boolean; aiReady: boolean; focusTarget: string | null; onRegenerate: (assetId: string) => void; onReplace: (assetId: string, file: File) => void }) {
  ...
  return <div className="asset-gallery">{assetKinds.map((kind) => {
    ...
    return <article id={`asset-${asset.id}`} tabIndex={-1} className={`creative-asset asset-${asset.kind} ${blocked ? "asset-blocked" : ""} ${legacyRoleReview ? "asset-needs-review" : ""} ${focusTarget === `asset-${asset.id}` ? "focus-target" : ""}`} key={asset.id}><div><ResilientImage src={appPath(asset.url)} alt={purpose.nameZh} />{blocked && <span className="asset-overlay">缺少商品信息 · 不能导出</span>}{legacyRoleReview && <span className="asset-overlay asset-overlay-warning">需要重新检查用途</span>}</div><footer><div className="asset-copy-meta"><b>{purpose.nameZh}</b><span className={blocked ? "asset-status blocked" : legacyRoleReview ? "asset-status pending" : `asset-status ${asset.complianceStatus}`}>{status}</span><p>{blocked ? requirement.reasonZh : legacyRoleReview ? "该素材由旧版本生成，重新检查后才能确认它真的符合用途。" : purpose.description}</p><small>{asset.factIds?.length ?? requirement.factIds.length} 条商品信息 · {blocked ? "补充信息后重新检查" : legacyRoleReview ? "等待用途检查" : `一致性 ${Math.round(asset.consistencyScore * 100)}%`}</small></div><div className="asset-actions"><button type="button" disabled={busy || !aiReady || !workspace.truth.confirmedAt || blocked} onClick={() => onRegenerate(asset.id)}>{busy ? "处理中…" : "重新生成"}</button><label className={`asset-replace ${busy || !workspace.truth.confirmedAt ? "disabled" : ""}`}>在此处替换<input type="file" accept="image/*" disabled={busy || !workspace.truth.confirmedAt} onChange={(event) => { const file = event.target.files?.[0]; if (file) onReplace(asset.id, file); event.target.value = ""; }} /></label></div></footer></article>;
  })}</div>;
}
...
function PagePreview({ workspace, channel }: { workspace: ProjectWorkspace; channel: Channel }) {
  ...
  return <div className="page-browser"><header><i /><i /><i /><span>{channelNames[channel]} / 中英双语详情页预览</span></header><div className="page-proof"><strong>真实素材已嵌入预览</strong><span>图片、文案与商品事实保持同一版本，可直接检查购买决策路径。</span></div>{modules.map((module, index) => {
    ...
    return <section className={`detail-module-preview detail-module-${module.type} ${index === 0 ? "detail-module-first" : ""}`} key={module.id}>{asset && <div className="detail-module-media"><ResilientImage src={appPath(asset.url)} alt={`${assetPurpose[asset.kind].nameZh} · ${module.title}`} /><span>{assetPurpose[asset.kind].nameZh}</span></div>}<div className="detail-module-copy"><small>{detailTypeName(module.type)} · 第 {index + 1} 个模块</small><h2>{module.title}</h2><p>{module.body}</p><div className="module-translation"><b>中文对照</b><h3>{module.titleZh || "中文标题尚未生成"}</h3><p>{module.bodyZh || "中文译稿尚未生成，请重新生成该渠道内容。"}</p></div>{module.factIds.length > 0 && <div className="module-facts"><span>商品信息来源</span>{module.factIds.map((factId) => <em key={factId}>{factNames.get(factId) || "商品信息已更新，请重新生成"}</em>)}</div>}</div></section>;
  })}</div>;
}
...
function ComplianceCenter({ workspace, busy, aiReady, onScan, onFix, onOptimizeFinding, onOptimizeAll, onLocate }: { workspace: ProjectWorkspace; busy: boolean; aiReady: boolean; onScan: () => void; onFix: () => void; onOptimizeFinding: (finding: ComplianceFinding) => void; onOptimizeAll: () => void; onLocate: (finding: ComplianceFinding) => void }) {
  ...
  const canExport = Boolean(workspace.truth.confirmedAt) && hasCompletedCheck && workspace.listings.every((listing) => listing.title.trim()) && open.length === 0 && unreviewedAssets.length === 0;
  const downloadExport = async () => {
    setExporting(true); setExportError("");
    try {
      const response = await fetch(appPath(`/api/projects/${workspace.project.id}/export`));
      if (!response.ok) {
  ...
  return <section className="focused-space publish-space"><SpaceIntro number="03" label="检查与导出" title="先检查清楚，再导出可用内容。" text="检查会说明发现了什么、依据哪条平台或法规要求，以及应该怎样修改。未覆盖的品类会明确提示人工复核。" />
    <div className="publish-overview"><div className="readiness-copy"><span>{open.length ? "需要处理" : "等待检测或已通过"}</span><h2>{open.length ? `${open.length} 项风险` : "当前没有未处理发现"}</h2><p>这是上线前的风险筛查，不代替平台审核、检测认证或法律意见。{failedAssets.length ? ` ${failedAssets.length} 张图片需要重新生成或替换。` : ""}</p></div><button className="main-action" disabled={busy || !hasGeneratedContent(workspace) || workspace.listings.some((listing) => !listing.title.trim())} onClick={onScan}>{busy ? "检测中…" : "开始检查"}</button></div>
    <div className="publish-grid"><section className="channel-readiness"><header><span>检查了哪些内容</span><small>美国市场</small></header>{workspace.project.channels.map((item) => { const rules = rulesForChannel(workspace, item); return <article key={item}><div className="channel-logo">{channelNames[item][0]}</div><div><h3>{channelNames[item]}</h3><p>{coverageName(workspace.project.coverage[item])}</p><details><summary>查看检查项目 <span>{rules.length} 项</span></summary><ul>{rules.map((rule) => { const source = (workspace.sources.length ? workspace.sources : ruleSources).find((entry) => entry.id === rule.sourceId); return <li key={rule.id}><b>{ruleName(rule.id)}</b><span>{source?.authority} · {source?.title}</span></li>; })}</ul></details></div></article>; })}</section><section className="risk-focus"><header className="risk-header"><div><span>检查结果</span><small>{open.length} 项</small></div>{open.length ? <div className="risk-actions"><button className="main-action" onClick={onOptimizeAll} disabled={busy || !aiReady}>{busy ? "AI 优化并复查中…" : aiReady ? "一键优化全部并复查 →" : "配置 AI 后可一键优化"}</button><button className="secondary-action" onClick={onFix} disabled={busy}>只清理文字并复查</button></div> : null}</header><div className="risk-list">{open.length ? open.map((finding) => { const source = workspace.sources.find((item) => item.id === finding.sourceId); return <article key={finding.id}><i className={finding.severity}>!</i><div><span>{severityName(finding.severity)} · {finding.location ? locationLabel(finding.location) : finding.target}</span><h3>{finding.excerpt}</h3><p className="finding-translation">中文：{finding.excerptZh || finding.excerpt}</p><p>{finding.explanationZh || finding.explanation}</p><strong>建议：{finding.suggestionZh || finding.suggestion}</strong><div className="finding-actions">{finding.location ? <><button type="button" className="finding-locate" onClick={() => onLocate(finding)}>定位并编辑：{locationLabel(finding.location)} →</button><button type="button" className="finding-optimize" disabled={busy || !aiReady} onClick={() => onOptimizeFinding(finding)}>{aiReady ? "AI 优化此项并复查" : "配置 AI 后可优化"}</button></> : <span className="finding-legacy">旧版记录，请重新检查后定位</span>}{source ? <a href={source.url} target="_blank" rel="noreferrer">查看规则来源 ↗</a> : <span>规则来源缺失</span>}</div></div></article>; }) : <div className="empty-panel"><b>暂无风险记录</b><p>请先开始检查；没有记录不代表已经完成审核。</p></div>}</div></section></div>
    <DeliveryGuide workspace={workspace} />
    <div className="launch-bar"><div><span>分平台最终交付包</span><b>Amazon · TikTok Shop · Shopify 各自独立文件夹</b><small>运营只需打开对应平台的 START-HERE.txt、01-listing-copy.csv 和 02-image-order.csv；审计 JSON 已移入 audit / technical。</small>{exportError && <em className="export-error" role="alert">导出错误：{exportError}</em>}</div>{open.length ? <button className="main-action" disabled>处理上方风险后再导出</button> : canExport ? <button className="main-action" disabled={exporting} onClick={downloadExport}>{exporting ? "正在打包导出…" : "下载分平台交付 ZIP →"}</button> : <button className="main-action" disabled>{failedAssets.length ? "请重新生成或替换图片后复检" : !hasCompletedCheck ? "先完成检查，再导出" : "完成事实确认、内容和图片复检后可导出"}</button>}</div>
  </section>;
}
...
function strategyName(strategy: "seo" | "brand" | "conversion") { return ({ seo: "搜索优化", brand: "品牌表达", conversion: "转化优先" } as const)[strategy]; }
function detailTypeName(type: string) { return ({ hero: "首屏介绍", benefits: "核心优势", scenario: "使用场景", comparison: "特征对比", specs: "规格信息", steps: "使用步骤", faq: "常见问题", reason: "选择理由" } as Record<string, string>)[type] ?? "详情模块"; }
function severityName(severity: "high" | "medium" | "low") { return ({ high: "高风险", medium: "中风险", low: "低风险" } as const)[severity]; }
...
function rulesForChannel(workspace: ProjectWorkspace, channel: Channel) {
  ...
  return complianceRules.filter((rule) => rule.channels.includes(channel) && (rule.categories.includes("*") || rule.categories.some((item) => item === category)));
}
function ruleName(ruleId: string) {
  return ({ "claim-absolute": "绝对化和保证性用语", "fact-duration": "时长、数量等数字信息", "health-treatment": "健康或治疗效果表述", "amazon-main-promo": "Amazon 主图要求", "tiktok-title-clickbait": "TikTok 标题诱导用语", "asset-role-facts": "图片用途和商品信息一致", "visual-asset-review": "图片真实性和用途检查" } as Record<string, string>)[ruleId] ?? "商品内容和平台要求";
}
