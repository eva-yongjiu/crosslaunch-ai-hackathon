  ...
  return <main className="experience-shell real-app">
    <header className="experience-header">
      ...
      <div className="experience-head-actions">
        <span className={`live-state ${runtime?.database === "available" ? "d1" : "fixture"}`}>{runtime?.database === "available" ? "D1 已连接" : "存储未连接"}</span>
...
function ComplianceCenter({ workspace, busy, onScan, onFix }: { workspace: ProjectWorkspace; busy: boolean; onScan: () => void; onFix: () => void }) {
  ...
  const canExport = Boolean(workspace.truth.confirmedAt) && workspace.listings.every((listing) => listing.title.trim()) && high.length === 0;
  return <section className="focused-space publish-space"><SpaceIntro number="03" label="COMPLIANCE & EXPORT" title="检测结果必须有依据，导出必须过门禁。" text="规则扫描使用当前保存内容。没有专项规则包的品类只显示部分覆盖，不会声称完全合规。" />
    <div className="publish-overview"><div className="readiness-copy"><span>{open.length ? "需要处理" : "等待检测或已通过"}</span><h2>{open.length ? `${open.length} 项风险` : "当前没有未处理发现"}</h2><p>此功能是风险筛查工具，不替代平台审核、检测认证或法律意见。</p></div><button className="main-action" disabled={busy || workspace.listings.some((listing) => !listing.title.trim())} onClick={onScan}>{busy ? "检测中…" : "运行真实规则检测"}</button></div>
    ...
    <div className="launch-bar"><div><span>最终交付包</span><b>Listing · 详情页 · 图片清单 · 合规报告 · 生成记录</b></div>{open.length ? <button className="main-action" onClick={onFix} disabled={busy}>删除不支持宣称并复检</button> : canExport ? <a className="main-action" href={`/api/projects/${workspace.project.id}/export`}>下载真实项目 ZIP →</a> : <button className="main-action" disabled>完成事实确认与 Listing 后可导出</button>}</div>
  </section>;
}
