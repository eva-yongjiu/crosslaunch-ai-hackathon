...
const navNames: Record<UiLanguage, string[]> = { "zh-CN": ["项目", "商品事实", "内容创作", "合规导出"], "en-US": ["Projects", "Product facts", "Content", "Compliance & export"] };
    ...
    for (;;) {
      ...
      try {
        ...
        if (result.task.status === "completed" || result.task.status === "failed") {
          ...
          else setMessage(result.task.outputSummary || (remaining ? `${t("后台处理完成，仍有 ", "Background processing finished with ")}${remaining}${t(" 项风险。", " risks remaining.")}` : t("后台处理完成，复检已通过。", "Background processing finished and the recheck passed.")));
          return;
        }
      } catch (cause) {
...
function TruthWorkbench({ language, workspace, setWorkspace, busy, aiReady, analyzing, hasUnsavedChanges, onUpload, onAnalyze, onConfirm, onSave }: { language: UiLanguage; workspace: ProjectWorkspace; setWorkspace: (value: ProjectWorkspace) => void; busy: boolean; aiReady: boolean; analyzing: boolean; hasUnsavedChanges: boolean; onUpload: (file: File) => void; onAnalyze: () => void; onConfirm: () => void; onSave: () => void }) {
  ...
  return <section className="focused-space"><SpaceIntro number="01" label={t("商品信息", "Product information")} title={t("先确认真实商品信息，再让 AI 创作。", "Confirm real product information before creating with AI.")} text={t("你可以上传原图让 AI 识别，也可以手动录入。只有你确认过的信息才会用于生成。", "Upload an original image for AI recognition or enter facts manually. Only confirmed information is used for generation.")} />
      <div className="workbench-grid"><label className="product-stage"><input type="file" accept="image/*" disabled={busy} onChange={(event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (file) onUpload(file); event.target.value = ""; }} />{workspace.truth.sourceAsset ? <ResilientImage language={language} src={appPath(workspace.truth.sourceAsset.url)} alt={t("商品原图", "Original product image")} /> : <div className="upload-placeholder"><b>{t("上传真实商品原图", "Upload the original product image")}</b><span>JPG / PNG / WEBP · {t("最大 8MB", "max 8MB")}</span></div>}<span className="stage-chip">{t("原始商品素材", "Original asset")}</span><button type="button">{workspace.truth.sourceAsset ? t("更换图片", "Replace image") : t("选择图片", "Choose image")}</button></label>
      <section className="truth-editor real-editor"><header><div><span>{t("商品事实档案", "Product fact profile")}</span><input className="title-input" value={workspace.truth.productName} onChange={(event) => updateTruth({ productName: event.target.value })} /></div><div className="truth-header-tools"><strong>{verified}<small>{t("已确认事实", "verified facts")}</small></strong></div></header>
        ...
        <div className="truth-actions"><button className="quiet-action" onClick={onAnalyze} disabled={busy || !aiReady || !workspace.truth.sourceAsset}>{analyzing ? t("正在识别商品事实…", "Recognizing product facts…") : aiReady ? t("AI 识别原图", "Recognize original image") : t("AI 未配置", "AI not configured")}</button><button className="quiet-action" onClick={addFact} disabled={busy}>＋ {t("手动添加事实", "Add fact manually")}</button></div>
         {(analyzing || workspace.truth.attributes.length > 0) && <div className={`truth-status-row ${analyzing ? "loading" : ""}`} role="status" aria-live="polite"><strong>{analyzing ? t("AI 正在分析商品图片", "AI is analyzing the product image") : `${t("已识别 ", "Recognized ")}${workspace.truth.attributes.length}${t(" 条候选事实", " candidate facts")}`}</strong><span>{analyzing ? t("正在调用视觉模型，请稍候，不要重复点击。", "The vision model is working. Please wait and do not click again.") : t("请逐条检查事实来源，再将确认项改为“已确认”。", "Review each fact and mark only verified items as confirmed.")}</span></div>}
         <div className="fact-table">{workspace.truth.attributes.map((fact) => <div className="fact-row" key={fact.id}><div className="fact-copy"><input aria-label={t("事实名称（英文）", "Fact name (English)")} value={fact.name} placeholder={t("属性，如：Material", "e.g. Material")} onChange={(event) => updateFact(fact.id, { name: event.target.value })} /><input aria-label={t("事实名称（中文）", "Fact name (Chinese)")} value={fact.nameZh ?? ""} placeholder={t("中文属性名", "Chinese name")} onChange={(event) => updateFact(fact.id, { nameZh: event.target.value })} /></div><div className="fact-copy"><input aria-label={t("事实值（英文）", "Fact value (English)")} value={fact.value} placeholder={t("真实值", "Verified value")} onChange={(event) => updateFact(fact.id, { value: event.target.value })} /><input aria-label={t("事实值（中文）", "Fact value (Chinese)")} value={fact.valueZh ?? ""} placeholder={t("中文真实值", "Chinese value")} onChange={(event) => updateFact(fact.id, { valueZh: event.target.value })} /></div><select aria-label={t("事实状态", "Fact status")} value={fact.status} onChange={(event) => updateFact(fact.id, { status: event.target.value as ProductFact["status"] })}><option value="pending">{t("待确认", "Pending")}</option><option value="verified">{t("已确认", "Confirmed")}</option><option value="missing">{t("缺失", "Missing")}</option></select><button aria-label={t("删除事实", "Delete fact")} onClick={() => updateTruth({ attributes: workspace.truth.attributes.filter((item) => item.id !== fact.id) })}>×</button></div>)}{!workspace.truth.attributes.length && <div className="empty-panel"><b>{t("尚无商品事实", "No product facts yet")}</b><p>{t("上传图片后调用 AI 识别，或手动添加可证明的事实。", "Upload an image for AI recognition, or add verifiable facts manually.")}</p></div>}</div>
        <footer><p><b>{workspace.truth.confirmedAt ? t("档案已确认", "Profile confirmed") : verified ? `${verified}${t(" 条事实已标记确认", " facts marked confirmed")}` : t("请先确认至少一条事实", "Confirm at least one fact first")}</b><span>{workspace.truth.confirmedAt ? t("已进入创作阶段，可从顶部导航返回查看。", "Creation is unlocked; use the top navigation to continue.") : t("下拉选择“已确认”后，才能进入创作；待确认事实不会被用于生成。", "Mark facts as confirmed before creation; pending facts are not used.")}</span><small id="truth-confirm-hint" className={`truth-confirm-hint ${canConfirm ? "ready" : ""}`}>{workspace.truth.confirmedAt ? t("事实档案已确认。", "Fact profile confirmed.") : analyzing ? t("AI 识别完成后，请先审核候选事实。", "Review the candidate facts after AI finishes.") : verified ? t("可以确认并进入创作。", "You can confirm and continue.") : t("当前按钮不可用：请将至少一条事实改为“已确认”。", "This button is disabled until one fact is confirmed.")}</small></p><div><button className="quiet-action" disabled={busy || !hasUnsavedChanges} onClick={onSave}>{hasUnsavedChanges ? t("保存草稿", "Save draft") : t("已保存", "Saved")}</button><button className="main-action" aria-describedby="truth-confirm-hint" disabled={!canConfirm} onClick={onConfirm}>{workspace.truth.confirmedAt ? t("事实已确认", "Facts confirmed") : t("确认事实并进入创作 →", "Confirm facts and continue →")}</button></div></footer>
      </section></div>
  </section>;
}
