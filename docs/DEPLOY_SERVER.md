...
6. 进入合规检测，先选择一个平台，只查看该平台的文字高亮、图片区域定位、规则来源和覆盖状态；
7. 点击“优化该平台全部风险并复查”或逐条处理，确认该平台风险归零或明确保留人工审核项；
8. 在底部对应平台卡片下载该平台发布包；三个平台必须分别检查、分别下载，不能把一个平台的通过结果当作另外两个平台的通过结果；
9. 验证导出内容：Amazon/TikTok Shop 使用发布字段表和图片顺序表复制到官方类目模板，Shopify 使用产品导入 CSV/HTML；运营人员不需要处理内部 JSON。
